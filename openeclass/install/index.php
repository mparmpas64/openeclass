...
if(isset($_REQUEST['install2']) OR isset($_REQUEST['back2']))
{
	...
	<input type=\"text\" hidden name=\"token\" value=".$_SESSION["token"]." > <input type=\"text\" hidden name=\"token\" value=".$_SESSION["token"]." > </form>";
	draw($tool_content);
}
...
elseif(isset($_REQUEST['install7']))
{
	...
	if (mysql_errno() > 0) // problem with server
	{
		...
		<input type=\"submit\" name=\"install3\" value=\"< $langBackStep3\"><input type=\"text\" hidden name=\"token\" value=".$_SESSION["token"]." > <input type=\"text\" hidden name=\"token\" value=".$_SESSION["token"]." > </form>";
		draw($tool_content);
		exit();
	}
...
//'.($MCU==''?'//':'').'$MCU = "'.$MCU.'";
    ...
    <form action=\"../\"><input type=\"submit\" value=\"$langEnterFirstTime\"><input type=\"text\" hidden name=\"token\" value=".$_SESSION["token"]." > <input type=\"text\" hidden name=\"token\" value=".$_SESSION["token"]." > </form>";
...
elseif (isset($_REQUEST['install1']) || isset($_REQUEST['back1']))
{
	...
	if($configErrorExists) {
		...
		$tool_content .= "<input type=\"text\" hidden name=\"token\" value=".$_SESSION["token"]." > <input type=\"text\" hidden name=\"token\" value=".$_SESSION["token"]." > </form>";
		draw($tool_content);
		exit();
	}
	...
	<td><form name='langform' action='$_SERVER[PHP_SELF]' method=\"post\">".selection($langLanguages, 'lang', $lang, 'onChange="document.langform.submit();"')."<input type=\"text\" hidden name=\"token\" value=".$_SESSION["token"]." > </form></td>
	</tr>
	<tr>
