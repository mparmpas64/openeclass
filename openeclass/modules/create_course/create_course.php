...
if (isset($_POST['create_course'])) {
	...
	if ( !isset($_REQUEST["token"]) ) {
		...
		$course_addon=htmlspecialchars($course_addon);
		$course_keywords=htmlspecialchars($course_keywords);
        ...
        $stmt = $mysqli -> prepare("INSERT INTO cours_user SET
                        cours_id = ?,
                        user_id = ?,
                        statut = '1',
                        tutor='1',
        ...
        $stmt = $mysqli -> prepare("INSERT INTO cours_faculte SET
                        faculte = ?,
                        code = ?,
                        facid = ?"); 
...
$tool_content .= "<input type=\"text\" hidden name=\"token\" value=".$_SESSION["token"]." > </form>";
