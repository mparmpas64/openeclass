...
if ($is_adminOfCourse && @$action == 1) {//upload html file
	...
	<br><input type=\"text\" hidden name=\"token\" value=".$_SESSION["token"]." > <input type=\"text\" hidden name=\"token\" value=".$_SESSION["token"]." > </form>";
	draw($tool_content, 2, 'course_tools');
	exit();
}
...
if ($is_adminOfCourse && @$action == 2) {//add external link
	...
	</table><input type=\"text\" hidden name=\"token\" value=".$_SESSION["token"]." > <input type=\"text\" hidden name=\"token\" value=".$_SESSION["token"]." > </form>";
	draw($tool_content, 2, 'course_tools');
	exit();
}
//---------------------------------------------------------
