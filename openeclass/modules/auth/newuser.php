...
if (!isset($submit)) {
	...
	$stmt->bind_param("sssssds",htmlspecialchars($nom_form),htmlspecialchars($prenom_form),htmlspecialchars($uname),$password_encrypted,$email,$department,htmlspecialchars($am));
	$stmt->execute();
	$inscr_user = $stmt->get_result();
	$stmt->close();
	...
	$tool_content .= "<table width='99%'><tbody><tr>" .
		...
		$prenom_form=htmlspecialchars($_POST[prenom_form]);
		$nom_form=htmlspecialchars($_POST[nom_form]);
