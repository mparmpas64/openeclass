...
if (isset($_POST["submit"])) {
    ...
    if ( !isset($_REQUEST["token"]) ) {
        ...
        foreach ($changeCourse as $key => $value) {
                ...
                if (!in_array($cid, $selectCourse)) {
                        ...
                        $str= "DELETE FROM cours_user
                        WHERE statut <> 1 AND statut <> 10 AND
                        user_id = ? AND cours_id = ?";
	...
	$errorExists = false;
        foreach ($selectCourse as $key => $value) {
                ...
                $str= "SELECT fake_code, password FROM cours WHERE cours_id = ?";
                $stmt = $mysqli->prepare($str);
                $stmt->bind_param("d",$cid);
                $stmt->execute();
                $course_info = $stmt->get_result();
                $stmt->close();
...
function expanded_faculte($fac_name, $facid, $uid) {
	...
	// build a list of course followed by user.
        ...
        $str= "SELECT cours.code code_cours, cours.fake_code fake_code,
                                        cours.cours_id cours_id, statut
                                 FROM cours_user, cours
                                 WHERE cours_user.cours_id = cours.cours_id AND user_id = ?";
        $stmt = $mysqli->prepare($str);
        $stmt->bind_param("d",$uid);
        $stmt->execute();
        $usercourses = $stmt->get_result();
        $stmt->close();
	...
	// get the different course types available for this faculte
        ...
        $str= "SELECT DISTINCT type FROM cours
        WHERE cours.faculteid = ? AND cours.visible <> 0
        ORDER BY cours.type";
        $stmt = $mysqli->prepare($str);
        $stmt->bind_param("d",$facid);
        $stmt->execute();
