...
if (isset($_REQUEST['do']) && $_REQUEST['do'] == "go") {
	...
	$str = "SELECT `user_id`, `hash`, `password`, `datetime` FROM passwd_reset
			WHERE `user_id` = ?
			AND `hash` = ?
			AND TIME_TO_SEC(TIMEDIFF(`datetime`,NOW())) < 3600
			";
			$stmt = $mysqli->prepare($str);
			$stmt->bind_param("ss",mysqli_real_escape_string($mysqli,$userUID),mysqli_real_escape_string($GLOBALS["___mysqli_ston"],$hash));
			$stmt->execute();
			$res = $stmt->get_result();
			$stmt->close();
	...
	/***** Email address entry form *****/
        if (isset($email) and !email_seems_valid($email)) {
                $tool_content .= '<table width="99%"><tbody><tr><td class="caution">' .
                                '<p><strong>' . htmlspecialchars($lang_pass_invalid_mail) . '<br />&nbsp;<br />' .
                                '&nbsp;<br />&nbsp;<br /></strong></p>' .
				'</td></tr></tbody></table>';
        }
