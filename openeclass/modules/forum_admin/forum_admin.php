...
if(isset($forumgo)) {
	...
	if ($result and mysql_num_rows($result) > 0) {
		...
		while(list($forum_id, $forum_name, $forum_desc, $forum_access,
			...
			$tool_content .= "</tbody></table><input type=\"text\" hidden name=\"token\" value=".$_SESSION["token"]." > </form><br/>";
		} else {
	...
	if ( !isset($_REQUEST["token"]) ) {
		...
		</tr></thead></table><input type=\"text\" hidden name=\"token\" value=".$_SESSION["token"]." > </form>";
	}
	...
	if ( !isset($_REQUEST["token"]) ) {
		...
		$str= "INSERT INTO catagories VALUES (NULL, ? , NULL)";
		$stmt = $mysqli->prepare($str);
		$stmt->bind_param("s",htmlspecialchars($catagories));
		$stmt->execute();
		$stmt->close()
	...
	if ( !isset($_REQUEST["token"]) ) {
		...
		</table><input type=\"text\" hidden name=\"token\" value=".$_SESSION["token"]." > </form>
		<br/>
