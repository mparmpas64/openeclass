...
if (isset($submit) && (!isset($ldap_submit)) && !isset($changePass)) {
	...
	$str= "SELECT username FROM user WHERE username=?";
	...
	// everything is ok
		...
		$stmt = $mysqli->prepare($str);
		$stmt->bind_param("sssssd",htmlspecialchars($nom_form),htmlspecialchars($prenom_form),htmlspecialchars($username_form),htmlspecialchars($email_form),htmlspecialchars($am_form),$persoStatus);
		$result = $stmt->execute();
		$stmt->close();
...
if ((!isset($changePass)) || isset($_POST['submit'])) {
	...
	<input type=\"text\" hidden name=\"token\" value=".$_SESSION["token"]." > 
	</form>
   ";
}
