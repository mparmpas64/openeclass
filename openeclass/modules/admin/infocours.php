...
/*****************************************************************************
		DEAL WITH LANGFILES, baseThemeAndCheck, OTHER INCLUDES AND NAMETOOLS
******************************************************************************/
...
if (isset($submit))  {
  ...
  $titulaires=htmlspecialchars($titulaires);
  $intitule=htmlspecialchars($intitule);
  $facid=htmlspecialchars($facid);*/
  ...
  $stmt = $mysqli -> prepare("UPDATE cours SET faculte=?, titulaires=?, intitule=?, faculteid=? WHERE code= ?");

  $tmp=mysql_real_escape_string($_GET['c']);

  $stmt -> bind_param('sssss', $facname, $titulaires, $intitule, $facid, $tmp);
  $r= $stmt->execute();
	...
	if ($stmt->affected_rows > 0) {
     ...
     $stmt = $mysqli -> prepare("UPDATE cours_faculte SET faculte=?, facid=? WHERE code=?");
     $tmp=mysql_real_escape_string($_GET['c']);
     $stmt -> bind_param('sss', $facname, $facid, $tmp);
     $r= $stmt->execute();
