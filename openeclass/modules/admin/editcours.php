...
if (isset($c)) {
  ...
  $str= "SELECT * FROM cours WHERE code = ?";
  $stmt = $mysqli->prepare($str);
  $stmt->bind_param("s",htmlspecialchars($c));
  $stmt->execute();
  $sql = $stmt->get_result();
  $stmt->close();
  ...
  $str= "SELECT code,intitule,doc_quota,video_quota,group_quota,dropbox_quota
      FROM cours WHERE code=?";
    $stmt = $mysqli->prepare($str);
    $stmt->bind_param("s",htmlspecialchars($c));
    $stmt->execute();
    $res = $stmt->get_result();
