...
/*****************************************************************************
		DEAL WITH  baseThemeAndCheck, OTHER INCLUDES AND NAMETOOLS
******************************************************************************/
// Check if user is administrator and if yes continue
// Othewise exit with appropriate message
$require_admin = TRUE;
// Include baseThemeAndCheck
include '../../include/baseThemeAndCheck.php'; 	$token = $_SESSION["token"];
...
******************************************************************************/
	...
	<ul id='opslist'>
	<li><a href='$_SERVER[PHP_SELF]?token=".$token."&a=1'>".$langAdd."</a></li>
	</ul>
	</div>";
...
if (!isset($a)) {
	...
	for ($j = 0; $j < mysql_num_rows($sql); $j++) {
		...
		<a href='$_SERVER[PHP_SELF]?token=".$token."&a=2&c=".$logs['id']."'>
		<img src='../../images/delete.gif' border='0' title='$langDelete'></img></a>&nbsp;&nbsp;
		<a href='$_SERVER[PHP_SELF]?token=".$token."&a=3&c=".$logs['id']."'>
...
elseif ($a == 1)  {
	...
	if (isset($add)) {
		...
		$codefaculte= htmlspecialchars($codefaculte);
		$faculte= htmlspecialchars($faculte);

		mysql_select_db($mysqlMainDb);
		$mysqli = new mysqli("db","root", "1234", $mysqlMainDb);
		...
		$stmt1 = $mysqli -> prepare("SELECT * FROM faculte WHERE code= ? ");
		$stmt1 ->bind_param('s', $codefaculte);
		$r= $stmt1->execute();
		$result=$stmt1->get_result();
		$codefaculteRows= $result->num_rows;

		
		
		
		$stmt2 = $mysqli -> prepare("SELECT * FROM faculte WHERE name= ? ");
		$stmt2 ->bind_param('s', $faculte);
		$r= $stmt2->execute();
		$result=$stmt2->get_result();
		$faculteRows= $result->num_rows;
		...
		if (empty($codefaculte) or empty($faculte)) {
			...
			<a href=\"$_SERVER[PHP_SELF]?token=".$token."&a=1\">".$langReturnToAddFaculte."</a></p></center>";
			}
		// Check for greek letters
		elseif (!preg_match("/^[A-Z0-9a-z_-]+$/", $codefaculte)) {
			$tool_content .= "<p>".$langGreekCode."</p><br />";
			$tool_content .= "<center><p><a href=\"$_SERVER[PHP_SELF]?token=".$token."&a=1\">".$langReturnToAddFaculte."</a></p></center>";
			}
		// Check if faculty code already exists
		elseif ($codefaculteRows> 0) {
			$tool_content .= "<p>".$langFCodeExists."</p><br />";
			$tool_content .= "<center><p><a href=\"$_SERVER[PHP_SELF]?token=".$token."&a=1\">".$langReturnToAddFaculte."</a></p></center>";
			}
		// Check if faculty name already exists
		elseif ($faculteRows > 0) {
			$tool_content .= "<p>".$langFaculteExists."</p><br />";
			$tool_content .= "<center><p><a href=\"$_SERVER[PHP_SELF]?token=".$token."&a=1\">".$langReturnToAddFaculte."</a></p></center>";
		} else {
		// OK Create the new faculty

			$stmt= $mysqli->prepare("INSERT into faculte(code,name,generator,number) VALUES(?,?,'100','1000')") or die ($langNoSuccess);

			//$stmt = $mysqli -> prepare("SELECT * FROM faculte WHERE name= ? ");
		...
		$tool_content .= "<form method=\"post\" action=\"".$_SERVER['PHP_SELF']."?token=".$token."&a=1\">";
		...
		<input type=\"text\" hidden name=\"token\" value=".$_SESSION["token"]." > </form>";
	}
...
elseif ($a == 2) {
	...
	$stmt1 = $mysqli -> prepare("SELECT * FROM cours WHERE faculteid= ? ");
	$stmt1 ->bind_param('i', $c);
	$r= $stmt1->execute();
	$result=$stmt1->get_result();
	...
	if ($rows > 0)  {
		...
		$stmt = $mysqli -> prepare("DELETE from faculte WHERE id=?");
		$stmt ->bind_param('i', $c);
		$r= $stmt->execute();
...
elseif ($a == 3)  {
	...
	if (isset($_POST['edit'])) {
		...
		mysql_select_db($mysqlMainDb);
		$mysqli = new mysqli("db","root", "1234", $mysqlMainDb);             
        $stmt2 = $mysqli -> prepare("SELECT * from faculte WHERE id <> ? AND name=? ");
		$stmt2 ->bind_param('is', $c ,$faculte);
		$r= $stmt2->execute();
		$result=$stmt2->get_result();
		$faculteRows= $result->num_rows;
		...
		if (empty($faculte)) {
			$tool_content .= "<p>".$langEmptyFaculte."</p><br>";
			$tool_content .= "<p align='right'><a href='$_SERVER[PHP_SELF]?token=".$token."&a=3&c=$c'>$langReturnToEditFaculte</a></p>";
			}
		...
		// OK Update the faculte
			$c = @intval($_REQUEST['c']);
			$stmt= $mysqli->prepare("UPDATE faculte SET name= ? WHERE id= ?");
			$stmt->bind_param('si', $faculte, $c);
			$stmt->execute();
        ...
        mysql_select_db($mysqlMainDb);
		$mysqli = new mysqli("db","root", "1234", $mysqlMainDb);             
        $stmt= $mysqli->prepare("SELECT code,name FROM faculte WHERE id= ?");
        $stmt->bind_param('i', $c);
        $stmt->execute();
        $result=$stmt->get_result();
		...
		$tool_content .= "<form method='post' action='$_SERVER[PHP_SELF]?token=".$token."&a=3'>";
		$tool_content .= "<table width='99%' class='FormData'>
		<tbody>
		<tr>
		<th width='220'>&nbsp;</th>
