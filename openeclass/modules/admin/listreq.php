...
if (!empty($show) && ($show=="closed")) {
	if (!empty($id) && ($id>0)) {
		...
		$str= "UPDATE prof_request set status='1', date_closed=NULL WHERE rid=?";
...
// ------------------------------
	...
	case '1':
		...
		$str= "UPDATE prof_request set status='2', date_closed=NOW() WHERE rid=?";
		$stmt = $mysqli->prepare($str);
		$stmt->bind_param("s",$id);
		$stmt->execute();
		$sql = $stmt->get_result();
	...
	case '2':
		...
		if ( !isset($_REQUEST["token"]) ) {
			...
			if (!empty($comment)) {
				...
				$str= "UPDATE prof_request set status = '3',
				date_closed = NOW(),
				comment = ?
				WHERE rid = ?";
				$stmt = $mysqli->prepare($str);
				$stmt->bind_param("ss",htmlspecialchars($comment),$id);
				$stmt->execute();
				$sql = $stmt->get_result();
				$stmt->close();
...
$langEmail: $emailhelpdesk";
			...
			$str= "SELECT comment, profname, profsurname, profemail, statut
			FROM prof_request WHERE rid = ?";
			$stmt = $mysql->prepare($str);
			$stmt->bind_param("s",$id);
			$stmt->execute();
			$r = $stmt->get_result();
			$stmt->close();
...
else
{
	...
	while ($req = mysql_fetch_array($sql)) {
		...
		$tool_content .= "<td align='center'>
		<a href='$_SERVER[PHP_SELF]?token=".$token."&id=$req[rid]&amp;close=1$reqtype' onclick='return confirmation();'>$langClose</a><br />
		<a href='$_SERVER[PHP_SELF]?token=".$token."&id=$req[rid]&amp;close=2$reqtype'>$langRejectRequest</a>";
