...
if (isset($_GET['delete'])) {
	...
	// submit announcement command
        if (isset($_POST['id'])) {
                ...
                $str= "UPDATE admin_announcements
                ...
                $stmt->bind_param("ssssss",htmlspecialchars($title),htmlspecialchars($newContent),htmlspecialchars($comment),htmlspecialchars($title_en),htmlspecialchars($newContent_en),htmlspecialchars($comment_en));
                $stmt->execute();
                $stmt->close();
                ...
                $str= "INSERT INTO admin_announcements
                SET gr_title = ?, gr_body = ?, gr_comment = ?,
                en_title = ?, en_body = ?, en_comment = ?,
                visible = '$visible', date = NOW()";
...
if ($displayAnnouncementList == true) {
        ...
        while ($myrow = mysql_fetch_array($result)) {
                ...
                &nbsp;&nbsp;
                <a href='$_SERVER[PHP_SELF]?token=".$token."&modify=$myrow[id]&amp;localize=$localize'>
                <img src='../../template/classic/img/edit.gif' title='$langModify' style='vertical-align:middle;' />
                </a>&nbsp;
                <a href='$_SERVER[PHP_SELF]?token=".$token."&delete=$myrow[id]&amp;localize=$localize' onClick='return confirmation();'>
