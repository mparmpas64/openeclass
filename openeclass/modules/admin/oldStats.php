...
if (!extension_loaded('gd')) {
  ...
  <tr>
    ...
    $str= "SELECT MONTH(start_date) AS month, YEAR(start_date) AS year, SUM(login_sum) AS visits FROM loginout_summary ".
    " WHERE ? GROUP BY MONTH(start_date)";
    $stmt = $mysqli->prepare($str);
    $stmt->bind_param("s",$date_where);
    $stmt->execute();
