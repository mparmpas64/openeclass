...
if (isset($submit) && $submit) {
	...
	if (($forum_type == 1) && !$user_logged_in && !$logging_in) {
		...
		</td></tr></table><input type=\"text\" hidden name=\"token\" value=".$_SESSION["token"]." > <input type=\"text\" hidden name=\"token\" value=".$_SESSION["token"]." > </form>";
		draw($tool_content, 2, 'phpbb', $head_content);
		exit();
	} else {
		if (!$uid AND !$fakeUid) {
	...
	<input type=\"text\" hidden name=\"token\" value=".$_SESSION["token"]." > </form><br/>";
}
