...
if (isset($submit) && $submit) {
	...
	$str= "INSERT INTO topics (topic_title, topic_poster, forum_id, topic_time, topic_notify, nom, prenom)
	VALUES (?, '$uid', '$forum', '$time', 1, '$nom', '$prenom')";
		$stmt = $mysqli->prepare($str);
		$stmt->bind_param("s",htmlspecialchars($subject));
		$stmt->execute();
		$result = $stmt->get_result();
		$stmt->close();


	$topic_id = mysql_insert_id($mysqli);
	...
	if (!$result = db_query($sql, $currentCourseID)) {
		...
		if ($post_id) {
			...
			$str= "INSERT INTO posts_text (post_id, post_text)
			VALUES ($post_id, ?)";
				$stmt = $mysqli->prepare($str);
				$stmt->bind_param("s",htmlspecialchars($message));
				$stmt->execute();
				$result = $stmt->get_result();
				$stmt->close();
